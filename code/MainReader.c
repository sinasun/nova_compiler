...
novaScript_long getFileSize(novaScript_string fname);
novaScript_intg isNumber(const novaScript_string ns);
novaScript_void startReader(novaScript_string, novaScript_string, novaScript_char, novaScript_intg, novaScript_intg);
...
novaScript_intg mainReader(novaScript_intg argc, novaScript_string* argv) {
	...
	novaScript_char mode = MODE_FIXED;
	novaScript_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
novaScript_void startReader(novaScript_string program, novaScript_string input, novaScript_char mode, novaScript_intg size, novaScript_intg increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	novaScript_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	novaScript_char symbol;			/* symbol read from input file */
...
novaScript_intg isNumber(const novaScript_string ns) {
	novaScript_char c; novaScript_intg i = 0;
	if (ns == NULL) return 0;
	while ((c = ns[i++]) == 0) {
